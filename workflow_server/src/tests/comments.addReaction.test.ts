/**
 * 🧪 [Domain: comments / Service: addReaction]
 * - 기능: 댓글 이모지 리액션 추가/토글 REST API 단위 테스트
 * - 경우의 수: 유효한 이모지 추가 성공 (201 Created), 필수 파라미터 누락 예외 (400 Bad Request)
 */

import { describe, it, expect, beforeAll, afterAll } from 'vitest';
import request from 'supertest';
import jwt from 'jsonwebtoken';
import { app } from '../app.js';
import { prisma } from '#lib/prisma.js';
import { createIssueService } from '../modules/issues/services/createIssue.service.js';

describe('🧪 [comments.addReaction] Service & REST API Unit Tests', () => {
  const jwtSecret = process.env.JWT_SECRET || 'antigravity-jwt-secret-key-2026';
  let testUser: { id: number; email: string };
  let authToken: string;
  let testProject: any;
  let testIssue: any;
  let testComment: any;

  beforeAll(async () => {
    testUser = await prisma.user.upsert({
      where: { email: 'reaction-user@example.com' },
      update: {},
      create: { email: 'reaction-user@example.com', name: 'Reaction User' }
    });
    authToken = jwt.sign({ userId: testUser.id, email: testUser.email }, jwtSecret, { expiresIn: '1h' });

    let status = await prisma.projectStatus.findFirst();
    if (!status) status = await prisma.projectStatus.create({ data: { name: 'Active', category: 'IN_PROGRESS' } });
    let priority = await prisma.projectPriority.findFirst();
    if (!priority) priority = await prisma.projectPriority.create({ data: { name: 'Medium', level: 2 } });

    testProject = await prisma.project.create({
      data: {
        name: 'Reaction Project',
        key: `RP_${Date.now()}`,
        ownerId: testUser.id,
        statusId: status.id,
        priorityId: priority.id
      }
    });

    testIssue = await createIssueService({ title: 'Reaction Issue', projectId: testProject.id, authorId: testUser.id });

    testComment = await prisma.comment.create({
      data: {
        content: 'Reaction target comment',
        issueId: testIssue.id,
        authorId: testUser.id
      }
    });
  });

  afterAll(async () => {
    await prisma.commentReaction.deleteMany({ where: { commentId: testComment.id } });
    await prisma.comment.delete({ where: { id: testComment.id } }).catch(() => {});
    await prisma.issue.delete({ where: { id: testIssue.id } }).catch(() => {});
    await prisma.project.delete({ where: { id: testProject.id } }).catch(() => {});
    await prisma.user.delete({ where: { id: testUser.id } }).catch(() => {});
  });

  describe('Case 1: 😊 댓글 리액션(이모지) 추가/토글 기능', () => {
    it('댓글에 이모지 리액션 추가 성공 시 리액션 정보가 반환되어야 한다 (201 Created)', async () => {
      const response = await request(app)
        .post(`/api/comments/${testComment.id}/reactions`)
        .set('Authorization', `Bearer ${authToken}`)
        .send({ userId: testUser.id, emoji: '👍' });

      expect(response.status).toBe(201);
      expect(response.body).toHaveProperty('emoji', '👍');
    });

    it('필수 이모지 파라미터 누락 시 400 Bad Request 에러를 반환해야 한다', async () => {
      const response = await request(app)
        .post(`/api/comments/${testComment.id}/reactions`)
        .set('Authorization', `Bearer ${authToken}`)
        .send({});

      expect(response.status).toBe(400);
    });
  });
});
