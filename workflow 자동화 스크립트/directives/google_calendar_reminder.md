...
## 1. 개요 및 목적
1. Google Calendar API를 연동하여 사용자의 구글 계정에 등록된 복수의 활성화된 캘린더들로부터 조회 당일의 일정(00:00:00 ~ 23:59:59)을 실시간으로 수집 및 가져옵니다.
...
### 다중 캘린더 관리 설정 (`calendar_config.json`)
스크립트를 최초 1회 실행하면 프로젝트 루트 디렉터리에 `calendar_config.json` 파일이 자동으로 생성되며 계정에 동기화된 모든 캘린더 목록을 취득해 저장합니다.
* 파일 구성 예시 (인코딩: `UTF-8 with BOM`):
```json
{
  "managed_calendars": [
    {
    ...
    {
      "id": "address@group.calendar.google.com",
      "summary": "회사 공유 업무 일정",
      "description": "업무 전용",
      "enabled": true
    }
...
```
* **캘린더 활성화/비활성화**: 모니터링을 진행하고 싶지 않은 특정 캘린더가 있다면 `"enabled": false`로 설정을 변경하면 감시 대상에서 즉각 제외됩니다.
...
```
*(참고: 특정 단일 캘린더만 수동 모니터링하려면 실행 명령어에 `--calendar-id <캘린더ID>` 옵션을 명시적으로 지정하여 실행할 수 있습니다.)*

## 3. 실행 방법

### 최초 실행 (인증 획득 및 설정 생성)
스크립트 최초 실행 시 OAuth 웹 인증과 캘린더 목록 수집이 진행됩니다. 아래 명령어로 실행합니다.
```powershell
python execution/google_calendar_reminder.py
```
- 실행 후 브라우저 창이 열리며, 본인의 Google 계정으로 로그인을 진행하고 액세스 권한을 허용해야 합니다.
- 인증이 완료되면 프로젝트 루트에 자동 갱신 가능한 `token.json` 파일이 생성됩니다.
- 이와 동시에 구글 캘린더 계정의 모든 활성 캘린더 목록을 긁어와 `calendar_config.json`을 새로 생성합니다. 
- 이후 실행부터는 기존 `token.json`과 `calendar_config.json`을 기준으로 자동 무인 동작합니다.

> [!WARNING]
> 만약 기존에 읽기 전용(`readonly`) 스코프로 생성된 `token.json` 파일이 프로젝트 루트에 존재하면, 쓰기 작업(CRUD)이 포함된 다른 스크립트 실행 시 권한 부족 에러(403 Forbidden)가 발생할 수 있습니다. 이 경우 반드시 기존 `token.json` 파일을 삭제하고 다시 실행하여 쓰기 권한이 포함된 새 로그인 인증을 진행해 주셔야 합니다.
